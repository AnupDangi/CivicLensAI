...
{tab === "TRANSCRIPT" && <>
             <div className="transcript-heading"><div><p className="fact-label">Detected languages</p><strong>{(analysis?.result?.detectedLanguages ?? []).filter((language) => language !== "und").length > 0 ? (analysis?.result?.detectedLanguages ?? []).filter((language) => language !== "und").join(" · ") : "English"}</strong></div><span>{liveTranscripts.length} saved/live · {transcriptArtifacts.length} source</span></div>
             {liveTranscripts.length > 0 && <article className="transcript-block live-transcript"><small><span className="status-dot"/> LiveKit room transcription</small><div className="live-transcript-lines">{liveTranscripts.slice().sort((a, b) => (a.startMs ?? 0) - (b.startMs ?? 0)).map((segment) => <p className={segment.final ? "final" : "interim"} key={`${segment.id}-${segment.startMs ?? 0}`}><strong>{segment.participant}</strong>{segment.text}</p>)}</div></article>}
